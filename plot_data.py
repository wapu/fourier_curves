...
cmap = LinearSegmentedColormap.from_list('my cmap', [(c2), 'white', c3])
...
def plot_dataset_example(model, limits=[-5,4,-4,5], n_samples=10000, seed=0):
    ...
    for i in range(4):
        if model.name == 'plus-shape':
            ...
            axes[i].fill(coords[:,0], coords[:,1], fc=(1,1,1,0), ec=c1, lw=2, zorder=-10)
    ...
    axes[4].imshow(corr, cmap=cmap, interpolation='nearest')
...
def show_parameter_sensitivity(limits=[-4,4,-4,4], n_samples=5, seed=0):
    ...
    for i in range(n_samples):
        ...
        axes[i][0].fill(coords[:,0], coords[:,1], fc=(1,1,1,0), ec=c1, lw=2, zorder=-10)
...
def plot_frog():
    ...
    points = np.array([[p['x'], p['y']] for p in points])
    points_dense = model.densify_polyline(points, 0.012)
    Ms = [1,2,3,4,6,7,11,16,21,30,50,100]
    Ms = list(range(1,50))
    coeffs = [model.fourier_coeffs(points, 2*i+1)[None,:,:] for i in Ms]
    curves = [model.trace_fourier_curves(c, 500)[0] for c in coeffs]
    curves = [curves[i] + np.array([0.5*i/len(Ms),-0.25*i/len(Ms)]) for i in range(len(curves))]

    fig = plt.figure(figsize=(6,6))

    for i in range(0,len(curves[0])-1):
        plt.plot([curves[j][i,0] for j in range(len(curves))], [curves[j][i,1] for j in range(len(curves))], lw=0.5, zorder=0, color=(0.21568627450980393, 0.49411764705882355, 0.7215686274509804,0.3))

    from matplotlib.colors import ColorConverter as cc
    green = np.array(cc.to_rgb('#96BF0D'))
    blue = np.array((0.21568627450980393, 0.49411764705882355, 0.7215686274509804))
    # blue = 0.25*green + 0.75*blue
    for i in range(len(curves)):
        plt.plot(curves[i][:,0], curves[i][:,1], lw=1+i/len(Ms), zorder=1, color=(i*green + (len(Ms)-i)*blue)/len(Ms))

    # plt.gca().set_xticks([]); plt.gca().set_yticks([])
    plt.axis('off')
    plt.subplots_adjust(left=.01, bottom=.01, right=.99, top=.99, wspace=.02, hspace=.01)
    plt.savefig(f'data/frog.pdf', bbox_inches='tight', pad_inches=0.05)
    plt.show()
