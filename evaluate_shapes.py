...
from data import LensShapeModel, CrossShapeModel
...
def cross_segments_from_params(params):
...
def points_to_cross_loss(points, params, corner_weight=1):
    # from shape parameters, extract all line segments
    segments = cross_segments_from_params(params)
...
def init_cross_params(angle, xshift=0, yshift=0, center=np.array([0,0])):
...
def fit_cross_shape_to_points(points, verbose=False):
    ...
    for xyshift in xyshifts:
        # Init cross shape parameters with dominant angle and center of the shape
        params = init_cross_params(angle, xshift=xyshift[0], yshift=xyshift[1], center=points.mean(axis=0))
        ...
        # Optimize parameters to fit cross shape
        for i in range(400):
            optim.zero_grad()
            loss = points_to_cross_loss(points, params, corner_weight=1-i/400)
            loss.backward()
            optim.step()
            lr_scheduler.step()
...
def iou_and_dice_cross_shape(params, points):
    segments = cross_segments_from_params(params)
    cross = geo.Polygon(segments.cpu().numpy()[:,0,:]).buffer(0)
    curve = geo.Polygon(points).buffer(0)
    intersection = cross.intersection(curve)
    union = cross.union(curve)
    return intersection.area / union.area, 2*intersection.area / (cross.area + curve.area)
...
def max_and_avg_hausdorff_distance_cross_shape(params, points):
    segments = cross_segments_from_params(params)
    cross = CrossShapeModel().densify_polyline(segments.cpu().numpy()[:,0,:], max_dist=0.02)
    return max_and_avg_hausdorff_distance(cross, points)



def check_cross_shape_fitting():
    # Create example Fourier cross shape
    model = CrossShapeModel()
    true_cross = model.generate_cross_shape()
    true_cross_fourier = model.fourier_coeffs(true_cross, n_coeffs=CrossShapeModel.n_parameters//4)
    cross_curve = model.trace_fourier_curves(true_cross_fourier[None,...])[0]
    cross_curve_dense = model.trace_fourier_curves(true_cross_fourier[None,...], n_points=1000)[0]
    points = torch.tensor(cross_curve).float().to(device)

    # # Distort a bit
    # points[:,1] += 0.4 * points[:,0]
    # cross_curve_dense[:,1] += 0.4 * cross_curve_dense[:,0]

    # Fit shape to points
    start = time()
    params = fit_cross_shape_to_points(points, verbose=True)
    iou, dice = iou_and_dice_cross_shape(params, points)
    print(f'\nIoU:  {iou:.3f}')
    print(f'DICE: {dice:.3f}')
    max_h, avg_h = max_and_avg_hausdorff_distance_cross_shape(params, cross_curve_dense)
    print(f'max Hausdorff: {max_h:.3f}')
    print(f'avg Hausdorff: {avg_h:.3f}')
    print(f'\nOptimization took {time() - start:.2f} seconds\n')

    # Plot result
    segments = cross_segments_from_params(params).cpu().numpy()
    plt.figure()
    for segment in segments:
        plt.plot(segment[:,0], segment[:,1], c=(0,0,0))
    plt.plot(cross_curve_dense[:,0], cross_curve_dense[:,1], c=(1,0,0))
    plt.axis('equal')
    plt.show()



def evaluate_cross_shape(x_sample):
    data_model = CrossShapeModel()

    curves = data_model.trace_fourier_curves(x_sample)
    curves_dense = data_model.trace_fourier_curves(x_sample, n_points=1000)

    results = {'IoU': [], 'DICE': [], 'max_h': [], 'avg_h': []}

    for i in range(len(curves)):
        points = torch.tensor(curves[i]).float().cpu()
        params = fit_cross_shape_to_points(points)
        iou, dice = iou_and_dice_cross_shape(params, points)
        max_h, avg_h = max_and_avg_hausdorff_distance_cross_shape(params, curves_dense[i])
    ...
    print(f'IoU: {np.mean(results['IoU'])}')
    print(f'DICE: {np.mean(results['DICE'])}')
    print(f'Hausdorff (max): {np.mean(results['max_h'])}')
    print(f'Hausdorff (avg): {np.mean(results['avg_h'])}')
...
def check_lens_shape_fitting():
    ...
    true_lens_fourier = model.fourier_coeffs(true_lens, n_coeffs=CrossShapeModel.n_parameters//4)
...
def evaluate_lens_shape(x_sample):
    data_model = LensShapeModel()
    ...
    for i in range(len(curves)):
        ...
        iou, dice = iou_and_dice_lens_shape(params, points)
        max_h, avg_h = max_and_avg_hausdorff_distance_lens_shape(params, curves_dense[i])
